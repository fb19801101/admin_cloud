...
<div class="site-text" id="demoForm" style="margin-top: 10px; display: none" target="testForm">
  <form class="layui-form" id="formLayer" onsubmit="return false">
    ...
    <div class="layui-form-item">
      <div class="layui-inline">
        <label class="layui-form-label label-width">交点坐标<br>NX</label>
        <div class="layui-input-inline">
          <input type="text" class="layui-input" id="ceNx" name="ceNx" lay-verify="required|number" autocomplete="off">
        </div>
      </div>
      <div class="layui-inline">
        <label class="layui-form-label label-width">交点坐标<br>EY</label>
        <div class="layui-input-inline">
          <input type="text" class="layui-input" id="ceEy" name="ceEy" lay-verify="required|number" autocomplete="off">
        </div>
      </div>
    </div>
...
<script>
    layui.use(['layer', 'form', 'table', 'upload', 'jquery'], function(){
        ...
        table.render({
            ...
            ,where: { field: "ce_jd", value: ""} //请求后端接口的条件，该处就是条件错误点，按照官方给出的代码示例，原先写成了 where: { key : { type: "all" } }，结果并不是我想的那样，如此写，key 将是后端的一个类作为参数，里面有 type 属性，如果误以为 key 是 Layui 提供的格式，那就大错特错了
            ,totalRow: true //开启合计行
            ...
            ,url: '${pageContext.request.contextPath}/software/searchCurveElement' // 使用url加载（可与data加载同时存在）
            ,cellMinWidth: 100
            ,event: true
            ,cols: [[{type:'numbers', title:'序号', fixed: true},{checkbox: true, fixed: true}
                ...
                ,{field:'ceNx', title:'交点坐标 NX', width:100} ,{field:'ceEy', title:'交点坐标 EY', width:100}
            ...
            switch(obj.event){
                ...
                case 'clearAllData':
                    table.reload('tableReload', {
                        method: 'post'
                        ,url: '${pageContext.request.contextPath}/software/clearCurveElement'
                        ,page: {
                            curr: 1 //重新从第 1 页开始
                        }
                    });
                ...
                case 'initCurveElement':
                    layer.open({
                        ...
                        ,yes: function(index){
                            ...
                            table.reload('tableReload', {
                                method: 'post'
                                ,url: '${pageContext.request.contextPath}/software/initCurveElement'
                                ,where: { dk_begin: $('#dk-begin').val(), dk_end: $('#dk-end').val()}
                                ,page: {
                                    curr: 1 //重新从第 1 页开始
                                }
                            });
                ...
                case 'layTable_Add':
                    layer.open({
                        ...
                        ,yes: function(index){
                            $.getJSON('${pageContext.request.contextPath}/software/insertCurveElement',{
                                ceJd: $('#ceJd').val()
                                ,ceDk: $('#ceDk').val()
                                ,ceNx: $('#ceNx').val()
                                ,ceEy: $('#ceEy').val()
                                ...
                                if(data.code === 1){
                                    ...
                                    table.reload('tableReload',{//重载表格
                                        method: 'post'
                                        ,url: '${pageContext.request.contextPath}/software/searchCurveElement'
                                        ,where: {
                                            field: 'ce_JD'
                                            ,value: $('#ceJd').val()
                                        }
            ...
            switch(obj.event){
                case 'calculate':
                    table.reload('calcReload', {
                        method: 'post'
                        ,url: '${pageContext.request.contextPath}/software/calculateCoordinate'
                        ,where: {
                            action: 0,
        ...
        table.on('tool(test)', function(obj){ //注：tool 是工具条事件名，test 是 table 原始容器的属性 lay-filter="对应的值"
            ...
            if(layEvent === 'detail'){
                ...
                layer.confirm('真的删除行么', function(index){
                    ...
                    layer.close(index);//关闭弹出层
                    //向服务端发送删除指令
                    $.getJSON('${pageContext.request.contextPath}/software/deleteCurveElement',{
                        jd: data.ceJd
                    },function(data){
                        //根据后台返回的参数，来进行判断
                        if(data.code === 1){
                ...
                layer.open({
                    ...
                    ,yes: function(index){
                        $.getJSON('${pageContext.request.contextPath}/software/modifyCurveElement',{
                            ceJd: $('#ceJd').val()
                            ,ceDk: $('#ceDk').val()
                            ,ceNx: $('#ceNx').val()
                            ,ceEy: $('#ceEy').val()
